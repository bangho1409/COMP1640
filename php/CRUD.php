...
class ideaCRUD{
  ...
  public function CreateIdea ($id_idea, $title, $content, $created_date, $last_modified_date, $id_category)
        {
  			    $succcess = -1;
              try {
                 global $connect;
                     $conn = mysqli_connect('localhost', 'root', '', 'webenterprise2');
  					...
  					$query = "INSERT INTO idea (id_idea,title,content,created_date,last_modified_date,id_category) VALUES ('{$id_idea}','{$title}','{$content}',NOW(),NOW(),'{$id_category}')";
            			...
            			try
            			   {
            				...
            				$result = mysqli_query($conn,$query);
            					while ($row = mysqli_fetch_row($result))
            					{
            						array_push($data, array("id_comment"=>$row[0], "content"=>$row[1], "created_date"=>$row[2], "last_modified_date"=>$row[3], "id_idea"=>$row[4]));
            					}
            					mysqli_close($conn);
